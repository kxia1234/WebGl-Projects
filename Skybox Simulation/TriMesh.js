 ...
 */
    ...
    */
    loaded(){
        world_s.push(glMatrix.mat4.clone(mvMatrix));
    ...
    */
    computeAABB(){
      var box = glMatrix.vec3.create();
      var total = this.fBuffer.length;
          for (var i = 1; i <= total; i++) {
              var current = this.fBuffer[i-1];
              this.getVertex(current, box);
              for (var j = 1; j < 4; j++) {
                  if (box[j-1] < this.minXYZ[j-1]) {
                      this.minXYZ[j-1] = box[j-1];
                  }
                  else {
                      this.maxXYZ[j-1] = box[j-1];
                  }
              }
          }
    }
    ...
    loadFromOBJ(fileText)
    {
       ...
       for (var i = 0; i < lines.length; i++) {
         if (lines[i][0] == '#') {
           ...
           if (tokens[0] == 'v') {
             for (var k = 1; k < 4; k++){
                 var temp_variable = parseFloat(tokens[k]);
                 this.vBuffer.push(temp_variable);

             }
           } 
           else if (tokens[0] == 'f') {
             for (var n = 1; n < 4; n++){
                 var temp_variable2 = parseInt(tokens[n]) - 1;
                  this.fBuffer.push(temp_variable2);
